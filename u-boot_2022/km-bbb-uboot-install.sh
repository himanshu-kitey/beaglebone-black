...
while [ ! -z "$1" ] ; do
        ...
        --scp)
			...
			# Ideally it should exist as it will contain scripts to install u-boot on target,
			# else that script should be copied to target as well.
			ssh ${username}@${ipaddress} "mkdir -p ~/install"
